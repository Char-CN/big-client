...
public class KamUserInfoController extends BaseController {
	...
	/*	User loginUser = (User) request.getSession().getAttribute("loginUser");
        ...
        if (!file.isEmpty()) {
            ...
            try {
                // 获取BcExcel对象
                KamExcel kamExcel = ExcelHandlerUtil.getExcelFile(file, request);

                kamExcel.setUserId(1L);
                kamExcel.setMtime(new Date());
                kamExcel.setCtime(kamExcel.getMtime());
                System.out.println("bcExcel = " + kamExcel);

                result.setObj(kamExcel);
                ...
                kamExtUserUploadService.importExcelData(listBean, kamExcel);

                // 上传文件
                UploadUtil.copy(file, kamExcel.getExcelRealPath(), kamExcel.getExcelRealName());
            } catch (Exception e) {
                result.setCode(AjaxResult.CODE_FAILURE);
