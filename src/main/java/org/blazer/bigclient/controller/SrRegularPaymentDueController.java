...
import org.blazer.bigclient.service.SrRegularPaymentDueService;
 ...
 */
@RequestMapping("/sr/regular_payment_due")
@Controller
public class SrRegularPaymentDueController extends BaseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(SrRegularPaymentDueController.class);

    @Autowired
    private SrRegularPaymentDueService srRegularPaymentDueService;
    ...
    public PageInfo<SrRegularPaymentDue> findByPage(HttpServletRequest request, HttpServletResponse response) {
        //获取前台传递过来的参数
        ...
        return this.srRegularPaymentDueService.findByPage(new SrRegularPaymentDue(),params,"redemptionDate");
    }
    ...
    public ModelAndView exportExcel(HttpServletRequest request) {
        ...
        try {
            ...
            //xml配置中的ID
            String id = "srRegularPaymentDue";
            // 要导出的数据
            List<SrRegularPaymentDue> list = this.srRegularPaymentDueService.findBySearch(new SrRegularPaymentDue(),params,"redemptionDate");
            if (list == null || list.size() == 0) {
                SrRegularPaymentDue regularPaymentDue = new SrRegularPaymentDue(0L,"空", "空", "空", "空","空", "空", "空",
                                                                                    "空", new Date(), new Date());
                list.add(regularPaymentDue);
            }
            //excel文件名称,不需要任何后缀
            String excelName = "RegularPaymentDue_Export_" + DateUtil.date2Str(new Date(), DateUtil.DEFAULT_DATE_TIME_FORMAT);
            //可以为空,自定义Excel头信息
            ...
            specifyFields.add("transferAmount");
