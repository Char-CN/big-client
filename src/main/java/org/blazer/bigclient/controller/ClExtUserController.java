...
import org.blazer.bigclient.service.ClExtUserService;
...
public class ClExtUserController extends BaseController {
    ...
    @Autowired
    private ClExtUserService clExtUserService;
    ...
    public PageInfo<ClExtUser> findByPage(HttpServletRequest request, HttpServletResponse response) {
        //获取前台传递过来的参数
    ...
    public AjaxResult saveOne(HttpServletRequest request) {
        ...
        try {
            //修改用户，先根据id查询到客户
            ClExtUser user = this.clExtUserService.selectByKey(id);
            user.setInvestmentAdviser(investmentAdviser);
            user.setMtime(new Date());
            this.clExtUserService.updateNotNull(user);
        } catch (Exception e) {
    ...
    public AjaxResult deleteUserById(HttpServletRequest request) {
        ...
        try {
            ClExtUser ClExtUser = this.clExtUserService.selectByKey(id);
            ClExtUser.setIfDelete(1);
            this.clExtUserService.updateNotNull(ClExtUser);
        } catch (Exception e) {
            result.setCode(AjaxResult.CODE_FAILURE);
    ...
    public ModelAndView exportExcel(HttpServletRequest request) {
        ...
        try {
            ...
            String id = "clExtUser";
            // 要导出的数据
            List<ClExtUser> list = this.clExtUserService.findBySearch(search);
            //excel文件名称,不需要任何后缀
