...
import org.blazer.bigclient.service.SrCrpPaymentDueService;
 ...
 */
@RequestMapping("/sr/crp_payment_due")
@Controller
public class SrCrpPaymentDueController extends BaseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(SrCrpPaymentDueController.class);

    @Autowired
    private SrCrpPaymentDueService srCrpPaymentDueService;
    ...
    public PageInfo<SrCrpPaymentDue> findByPage(HttpServletRequest request, HttpServletResponse response) {
        //获取前台传递过来的参数
        ...
        return this.srCrpPaymentDueService.findByPage(new SrCrpPaymentDue(),params,"redemptionDate");
    }
    ...
    public ModelAndView exportExcel(HttpServletRequest request) {
        ...
        try {
            ...
            //xml配置中的ID
            String id = "srCrpPaymentDue";
            // 要导出的数据
            List<SrCrpPaymentDue> list = this.srCrpPaymentDueService.findBySearch(new SrCrpPaymentDue(),params,"redemptionDate");
            if (list == null || list.size() == 0) {
                SrCrpPaymentDue crpPaymentDue = new SrCrpPaymentDue(0L,"空", "空", "空", "空","空", "空", "空",
                                                                                    "空", new Date(), new Date());
                list.add(crpPaymentDue);
            }
            //excel文件名称,不需要任何后缀
            String excelName = "CrpPaymentDue_Export_" + DateUtil.date2Str(new Date(), DateUtil.DEFAULT_DATE_TIME_FORMAT);
            //可以为空,自定义Excel头信息
